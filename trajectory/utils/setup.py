...
def convert_to_type(string):
    if isinstance(string, bool):
        return string
    if isinstance(string, str) and string.lower() in ("true", "false", "True", "False"):
...
class Parser(Tap):
    ...
    def add_extras(self, args):
        ...
        if len(extras) % 2 == 1:
            print(f'[ utils/setup ] Drop excessive extra arg: {extras[-1]}')
            extras = extras[:-1]
        ...
        for i in range(0, len(extras), 2):
            ...
            if not hasattr(args, key):
                print(f'[ utils/setup ] skipping {key} not found in config: {args.config}')
                continue
            ...
            if val == 'None':
                ...
                val = 'latest'
            elif old_type in [bool, type(None), list]:
                val = eval(val)
            else:
                val = val
            val = convert_to_type(val)
            setattr(args, key, val)
    ...
    def mkdir(self, args):
        if 'logbase' in dir(args) and 'dataset' in dir(args) and 'exp_name' in dir(args):
            ...
            if 'suffix' in dir(args):
                args.savepath = os.path.join(os.path.expanduser(args.savepath), str(args.suffix))
